...
class Roller(object):
    def __init__(self):
        """
        NOTE: Since the Roller is created in a class method, the window_size
        and emit_partials can't be passed in.  The RollingWindowChannel that
        creates these sets those values explicitly in its own initializer.

        Unless get_windower() can be made into an instance method...
        """
        self.window_size = 0
    ...
    def window(self, key, value):
        """
        This could theoretically emit groups as soon as they are the proper
        size, but it does it "the next time through" so that there is something
        remaining for tail() to do.
        """
        assert self.window_size > 0     # Make sure this got set!

        self.values.append(value)

        if len(self.values) > self.window_size:
            # Emit the group (minus the newest) and deduct the oldest going forward
            yield self.last_key, self.values[:-1]
            self.values = self.values[1:]
        elif self.emit_partials and (self.last_key is not None):
            yield self.last_key, self.values[:-1]
    ...
    def tail(self):
        ...
        # If there's anything that is still appropriate to emit, do so
        if self.values and (self.emit_partials or len(self.values) == self.window_size):
            yield self.last_key, self.values[:]


class RollingWindowChannel(WindowChannel):
    def __init__(self, channel, window_size, emit_partials=False, transform=None):
        """
        Group items from an input channel into rolling windows of a given size,
        optionally emitting the early windows that are less than the target size.

        Each group is emitted with the key from the last item added to the group.
        ...
        Parameters:
            ...
            emit_partials (boolean): True iff early rolling windows smaller than
                window_size should be emitted

            transform (callable): [OPTIONAL] a function that is called per input
                channel item (with that item as the sole argument), which should
                return an iterable collection of sortable, hashable window keys.

                If no ``transform`` function is given, the default behavior is
                to treat each item as a sequence, with the desired key in first
                position (``item[0]``).
        """
        super(RollingWindowChannel, self).__init__(channel, transform)
        # NOTE: These values are set here because get_windower() is a class method
        self.windower.window_size = window_size
        self.windower.emit_partials = emit_partials
